...
jpeg(filename=paste("flow", gage.names[i], ".jpg"), width = 600, height = 600, units = "px", pointsize = 12,
     quality = 300)


print(ggplot(pre_flow_site, aes(jdate, meanflow))+ 
  geom_line(data=pre_flow_site, aes(jdate, meanflow, color="1993-95")) +
  geom_line(data=post_flow_site, aes(jdate, meanflow, color="2017-19"))+
  geom_line(data=pre_flow_site, aes(y=rollmean(meanflow, 20, na.pad=TRUE), color="1993-95"), size=1.5, na.rm = TRUE) +
  geom_line(data=post_flow_site, aes(y=rollmean(meanflow, 20, na.pad=TRUE), color="2017-19"),size=1.5,na.rm = TRUE) +
    theme_bw() +
